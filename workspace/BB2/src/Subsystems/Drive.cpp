...
bool Drive::RunProfile( void ) {
	...
	if( cnt > 5 ) {
		...
		backLeft->Set( ControlMode::MotionProfile, 1 );
		backRight->Set( ControlMode::MotionProfile, 1 );
		...
		if( mpStatus.isLast ) {
			...
			backLeft->Set( ControlMode::MotionProfile, 2 );
			backRight->Set(ControlMode::MotionProfile, 2);
